...
def _control_state(*values):
    option = H3Acceleration.from_values(values)
    return (
        acceleration_note(*values),
        gr.update(visible=option.decode_mode == "fast"),
        gr.update(visible=option.attention == "sol"),
        gr.update(visible=option.attention == "sla"),
        gr.update(visible=option.attention != "dense"),
    )


def create_acceleration_controls():
    defaults = H3Acceleration()
    with gr.Accordion("高速化 · 任意設定 / 画質・メモリとの比較", open=False, elem_id="h3-acceleration"):
        gr.Markdown("4項目は独立して選択できます。まず1つずつ同じSeedで比較してください。標準設定は自動変更しません。")
        model = gr.Dropdown(
        ...
        reset = gr.Button("標準構成 + 20 Stepsに戻す", interactive=False, elem_id="h3-acceleration-reset")
        gr.Markdown(
    ...
    controls = [model, vae, decode, batch, attention, tau, keep, start]
