...
公開前の依存監査への対応として、Accelerateの未修正checkpointロードAPIを拒否し、SenseNovaは固定ソース・設定のハッシュ検証とオフライン読込を必須にしました。ライブラリ自体の未修正項目と期限付き例外は[セキュリティモデル](docs/security-model.md)に記載しています。
