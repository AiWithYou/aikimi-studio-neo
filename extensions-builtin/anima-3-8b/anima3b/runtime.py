...
class Anima3BRuntime:
    def __init__(self) -> None:
        ...
        self._v2_key: tuple[tuple[str, int, int], int] | None = None
    ...
    def _conditioning_cache_signature(
        ...
        if bundle_path is not None:
            return (
                V2_ARCHITECTURE,
                self._file_fingerprint(bundle_path),
                self._file_fingerprint(qwen_path),
                1.0,
                None if negative_strength is None else 1.0,
            )
    ...
    def _load_v2_models(
        ...
        native_adapter = native_clip.cond_stage_model.qwen3_06b.llm_adapter
        key = (self._file_fingerprint(bundle_path), id(native_adapter))
    ...
    def _encode_v2(self, native_engine, native_clip, prompt):
        ...
        placeholders = []
        run_ids = []
        for native, semantic in zip(native_rows, semantic_rows, strict=True):
    ...
    def encode(
        ...
        if strength is None or strength == 0.0:
            ...
            if self._active_bundle_metadata is not None:
                result = self._offload_conditioning(
                    result,
    ...
    def install(
        ...
        checkpoint_path = self._checkpoint_path(model)
        ...
        cache_signature = self._conditioning_cache_signature(
            ...
            strength,
            negative_strength,
            bundle_path=checkpoint_path if metadata is not None else None,
        )
    ...
    def restore(self, processing) -> None:
        ...
        was_v2 = (
            self._active_bundle_metadata is not None
            or self._v2_sampling_patcher is not None
        )
        if (
        ...
        self._v2_runs.clear()
        if was_v2:
            # Hires用も破棄し、次のリクエストで解放済みrun IDを再利用しない。
            for cache_name in ("cached_c", "cached_uc", "cached_hr_c", "cached_hr_uc"):
                if not hasattr(processing, cache_name):
                    continue
                cache = getattr(processing, cache_name, None)
                if isinstance(cache, list):
                    cache[:] = [None, None, None]
                setattr(processing, cache_name, [None, None, None])
