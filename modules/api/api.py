...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        self.add_api_route(
            "/sdapi/v1/forge-model-status",
            self.get_forge_model_status,
            methods=["GET"],
            response_model=models.ForgeModelStatus,
        )
        self.add_api_route(
            "/sdapi/v1/forge-model-status/ensure-loaded",
            self.ensure_forge_model_status,
            methods=["POST"],
            response_model=models.ForgeModelStatus,
        )
    ...
    def init_script_args(self, request, default_script_args, selectable_scripts, selectable_idx, script_runner, *, input_script_args=None):
        ...
        if selectable_scripts:
            overlay_script_args(script_args, selectable_scripts.args_from, selectable_scripts.args_to, request.script_args)
